            ...
            <el-tab-pane label="读取配置">
                ...
                <el-table :data="form.Read" style="width: 100%;">
                    ...
                    <el-table-column label="数据格式">
                        <template #default="scope">
                            <span v-if="scope.row.Config != null" style="margin-right: 10px;">{{ scope.row.Config
                            }}</span>
            ...
            <el-tab-pane label="属性配置" name="3">
                ...
                <el-table :data="form.Config" style="width: 100%;" @current-change="handleCurrentChange">
                    ...
                    <el-table-column label="类型">
                        <template #default="scope">
                            <el-cascader v-model="scope.row.DType" :options="form.Read" :props="cprops" />
...
const submitForm = (formEl) => {
    ...
    // formEl.validate((valid) => {
    //     if (valid) {
    //         if (rowIndex.value == -1 && rowID.value == "") {
    //             tableData.value.push({ ...form });
    //             //post("addType", { ...form })
    //         } else {
    //             var indexData;
    //             if (rowIndex.value > -1)
    //                 indexData = tableData.value[rowIndex.value];
    //             else if (rowID.value != "")
    //                 indexData = tableData.value.find((v, index, obj) => v.ID = rowID.value);
    //             indexData.Name = form.Name;
    //             indexData.Type = form.Type;
    //             indexData.Read = [...form.Read];
    //             indexData.Write = [...form.Write];
    //             //post("updateType", { ...form })
    //         }
    //         resetForm();
    //     } else {
    //         return false
...
const editRow1 = (data) => {
    ...
    for (var i = 0; i < data.Len; i++) {
        var dType = -1;
        if (data.Config != null && data.Config[i])
            dType = data.Config[i];
